...
# 读取数据
data = []
policy_delay_ms = None
policy_update_time = None
policy_effective_time = None

with open('../results/exp5/dynamic_policy_effect.txt', 'r') as f:
    for line in f:
        if line.strip() and not line.startswith('#'):
            ...
            elif line.startswith('POLICY_DELAY_MS:'):
                try:
                    policy_delay_ms = float(line.split(':')[1].strip())
                except:
                    pass
            elif line.startswith('POLICY_UPDATE_TIME_MS:'):
                try:
                    policy_update_time = float(line.split(':')[1].strip())
                except:
                    pass
            elif line.startswith('POLICY_EFFECTIVE_TIME_MS:'):
                try:
                    policy_effective_time = float(line.split(':')[1].strip())
                except:
                    pass
...
# 策略生效延迟标注
if policy_delay_ms is not None and policy_effective_time is not None:
    effective_time_sec = policy_effective_time / 1000
    # 绘制策略生效线
    ax1.axvline(x=effective_time_sec, color='#9b59b6', linestyle=':', linewidth=2, 
               label=f'Policy Effective (t={effective_time_sec:.1f}s)')
    # 绘制延迟区间
    ax1.annotate('', xy=(effective_time_sec, initial_quota - 0.5), 
                xytext=(update_time, initial_quota - 0.5),
                arrowprops=dict(arrowstyle='<->', color='#9b59b6', lw=2))
    ax1.text((update_time + effective_time_sec) / 2, initial_quota - 0.8, 
            f'Policy Delay\n{policy_delay_ms:.1f}ms', 
...
if policy_delay_ms is not None:
    print(f"\n策略下发延迟:")
    print(f"  策略下发时间: {policy_update_time/1000:.3f}s")
    print(f"  策略生效时间: {policy_effective_time/1000:.3f}s")
    print(f"  策略延迟: {policy_delay_ms:.1f}ms")
print(f"\n✓ 热更新验证成功!")
