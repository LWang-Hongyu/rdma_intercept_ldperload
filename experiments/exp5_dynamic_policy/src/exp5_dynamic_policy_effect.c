...
int main(int argc, char *argv[]) {
    ...
    // 计算策略生效延迟
    // 策略生效点：第一个在更新后成功的QP创建时间
    double policy_effective_time = -1;
    int first_success_after_update = -1;
    for (int i = update_at_qp - 1; i < MAX_QP; i++) {
        if (attempts[i].success) {
            policy_effective_time = attempts[i].timestamp_ms;
            first_success_after_update = i + 1;
            break;
        }
    }
    
    // 策略下发时间（估计值：第update_at_qp-1次尝试的时间）
    double policy_update_time = attempts[update_at_qp - 2].timestamp_ms;
    double policy_delay_ms = -1;
    if (policy_effective_time > 0) {
        policy_delay_ms = policy_effective_time - policy_update_time;
    }
    ...
    if (fp) {
        ...
        // 策略延迟信息
        fprintf(fp, "\n# 策略下发延迟分析\n");
        fprintf(fp, "POLICY_UPDATE_TIME_MS: %.3f\n", policy_update_time);
        if (policy_effective_time > 0) {
            fprintf(fp, "POLICY_EFFECTIVE_TIME_MS: %.3f\n", policy_effective_time);
            fprintf(fp, "POLICY_DELAY_MS: %.3f\n", policy_delay_ms);
            fprintf(fp, "FIRST_SUCCESS_AFTER_UPDATE: %d\n", first_success_after_update);
        } else {
            fprintf(fp, "POLICY_EFFECTIVE_TIME_MS: N/A\n");
            fprintf(fp, "POLICY_DELAY_MS: N/A\n");
            fprintf(fp, "FIRST_SUCCESS_AFTER_UPDATE: N/A\n");
        }
    ...
    if (success_after > 0) {
        ...
        if (policy_delay_ms > 0) {
            printf("  策略生效延迟: %.3f ms\n", policy_delay_ms);
            printf("  首次成功创建(更新后): 第%d次尝试\n", first_success_after_update);
        }
    } else {
        printf("\n✗ 热更新可能未生效\n");
