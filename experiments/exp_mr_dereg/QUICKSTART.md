...
```bash
cd ~/rdma_intercept_ldpreload/experiments/exp_mr_dereg
./run.sh all
```
...
```bash
# 场景A（无拦截）结果
cat results/victim_no_protection.csv
# 或查看图表
# results/victim_no_protection.png

# 场景B（有拦截）结果
cat results/victim_with_protection.csv
# 或查看图表
# results/victim_with_protection.png
```
...
```bash
cd ~/rdma_intercept_ldpreload/experiments/exp_mr_dereg
python3 src/victim_bw_monitor.py --mode=client --server=192.168.106.2 \
    --duration=30 --output=results/victim_no_protection.csv
```

**步骤3：5秒后启动Attacker（终端2）**
```bash
cd ~/rdma_intercept_ldpreload/experiments/exp_mr_dereg
./attacker --delay=5000 --duration=25000 --num_mrs=50 --batch_size=10
```

**步骤4：分析结果**
```bash
python3 analysis/analyze_bandwidth.py \
...
LD_PRELOAD=./build/librdma_intercept.so \
    python3 experiments/exp_mr_dereg/src/victim_bw_monitor.py \
    --mode=client --server=192.168.106.2 \
    --duration=30 --output=experiments/exp_mr_dereg/results/victim_with_protection.csv
```
...
cd ~/rdma_intercept_ldpreload
LD_PRELOAD=./build/librdma_intercept.so \
    ./experiments/exp_mr_dereg/attacker --delay=5000 --duration=25000 \
...
cd ~/rdma_intercept_ldpreload/experiments/exp_mr_dereg
python3 analysis/analyze_bandwidth.py \
    results/victim_with_protection.csv \
    results/victim_with_protection.png
```
...
| 攻击程序未编译 | `gcc -O2 -o attacker src/attacker.c -libverbs -lpthread` |
